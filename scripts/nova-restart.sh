...
#
# Usage:  ./nova-restart.sh <launchd-label>
# Example:./nova-restart.sh com.example.agent
...
for i in $(seq 1 10); do
  if pgrep -fl "${LABEL}\|nova-pty-runner\|agent-pty-runner" >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
...
RUNNER_PID="$(pgrep -f 'nova-pty-runner\|agent-pty-runner' | head -1 || true)"
